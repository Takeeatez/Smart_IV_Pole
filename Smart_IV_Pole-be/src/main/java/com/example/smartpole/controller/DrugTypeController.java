...
/**
 * DrugTypeController
 * - 약품 유형(DrugTypeEntity)에 대한 REST API 컨트롤러
...
public class DrugTypeController {
     ...
     * 전체 약품 목록 조회
     * GET /api/v1/drips
     * @return 모든 DrugTypeEntity 리스트
     */
    @GetMapping
    public ResponseEntity<List<DrugTypeEntity>> getAllDrugTypes() {
        // 모든 약품 유형을 조회하여 반환
        List<DrugTypeEntity> drugTypeEntities = drugTypeService.getAllDrugTypes();
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<DrugTypeEntity> getDrugTypeById(@PathVariable Integer id) {
        // ID로 약품 유형을 조회, 없으면 404 반환
        Optional<DrugTypeEntity> drugType = drugTypeService.getDrugTypeById(id);
     ...
     */
    @PostMapping
    public ResponseEntity<DrugTypeEntity> createDrugType(@RequestBody DrugTypeEntity drugTypeEntity) {
        // 약품 유형 신규 등록
        DrugTypeEntity savedDrugTypeEntity = drugTypeService.createDrugType(drugTypeEntity);
        return ResponseEntity.status(HttpStatus.CREATED).body(savedDrugTypeEntity);
    }
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<DrugTypeEntity> updateDrugType(@PathVariable Integer id, @RequestBody DrugTypeEntity drugTypeEntity) {
        // 약품 유형 정보 수정
        DrugTypeEntity updatedDrugTypeEntity = drugTypeService.updateDrugType(id, drugTypeEntity);
        return ResponseEntity.ok(updatedDrugTypeEntity);
    }
